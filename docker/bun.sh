...
# modified from https://bun.com/install
set -euo pipefail
...
if [[ $# -ne 1 ]]; then
    error 'Invalid number of arguments, usage: script <version>'
fi
...
bun_uri=https://registry.npmmirror.com/@oven/bun-$target/$1/files/bin/bun
...
curl --fail --location --progress-bar --output "$exe" "$bun_uri" ||
